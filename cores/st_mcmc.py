...
class MCMCSampler(nn.Module):
    ...
    def score_fn(self, x, x0hat, model, xt, sigma, gradient_rewards, mc_step):
        """
        ...
        for reward in gradient_rewards:
            if mc_step % reward.freq == 0:
                grad, value = reward.get_gradients(x)
                rewards_grad_term += reward.scale * grad
                sum_rewards += value

        rewards_grad_term *= 1 / self.tau ** 2

        return xt_term + prior_term + rewards_grad_term, sum_rewards
    ...
    def mc_prepare(self, x0hat, xt, model, sigma):
    ...
    def sample(self, xt, model, x0hat, sigma, ratio, gradient_rewards, record=False, verbose=False):
        """
        ...
        """
        if self.mc_algo == 'mh':
            return self.sample_mh(xt, model, x0hat, sigma, ratio, record, verbose)
        if record:
            self.trajectory = Trajectory()
        lr = self.get_lr(ratio)
        self.mc_prepare(x0hat, xt, model, sigma)
        self.prepare_prior_score(x0hat, xt, model, sigma)

        x = x0hat.clone().detach()
        pbar = tqdm.trange(self.num_steps) if verbose else range(self.num_steps)
        for mc_step in pbar:
            cur_score, fitting_loss = self.score_fn(x, x0hat, model, xt, sigma, gradient_rewards, mc_step)
