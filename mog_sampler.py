...
from cores.mog_mcmc import MCMCSampler
...
class DAPS(nn.Module):
    ...
    def sample(self, model, x_start, search_rewards, gradient_rewards, search, evaluator=None,
        ...
        for step in pbar:
            ...
            if evaluator and 'gt' in kwargs:
                with torch.no_grad():
                    gt = kwargs['gt']
                    x0hat_results = evaluator(gt=gt, pred=x0hat, text=text)
                    x0y_results = evaluator(gt=gt, pred=x0y, text=text)
...
class LatentDAPS(DAPS):
    ...
    """
    def sample(self, model, z_start, search_rewards, gradient_rewards, search, evaluator=None, 
                record=False, verbose=False, **kwargs):
        """
        ...
        for step in pbar:
            ...
            # Find the rewards and do the search
            rewards = torch.zeros(z0hat.shape[0], device=z0hat.device)
            for reward in search_rewards:
                rew = reward.get_reward(z0hat)
                if step > 50:
                    print('reward is: ', rew.cpu().detach().numpy())
                    print('reward mean: ', np.mean(rew.cpu().detach().numpy()))
                rewards += rew
            ...
            if search_rewards:  # if search rewards is empty we shouldn't do any search
                resampled_idx = search.search(rewards, step)
                zt = zt[resampled_idx].clone()
                z0hat = z0hat[resampled_idx].clone()
            ...
            if evaluator and 'gt' in kwargs:
                with torch.no_grad():
                    gt = kwargs['gt']
                    x0hat_results = evaluator(gt, x0hat, text=text)
                    x0y_results = evaluator(gt, x0y, text=text)
